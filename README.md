...
8. Set `TAXJAR_API_KEY=<live-token>` and `TAXJAR_SANDBOX=false` to register the TaxJar tax provider. Add **Washington** as a nexus state in the TaxJar dashboard (sandbox and live lists are separate) and enter the WA sales-tax permit number *before* the first live order. `TAXJAR_FROM_*` can be left unset — it falls back to `SHIPPO_FROM_*`. AutoFile is an optional per-state toggle in the TaxJar dashboard (~$35-50 per filing).
9. Run `npx medusa db:setup` + `npm run seed` once (from a local shell with prod env loaded) to populate the catalog. Replace dev `stockQuantity` values first too (look for `// TODO: replace with real prod starting inventory`). After seeding, evolve the catalog via the Admin UI or one-off `medusa exec` scripts — **never** re-run `npm run seed` against prod. If you seed before `TAXJAR_API_KEY` is set, the US tax region gets stuck on `tp_system` — fix with a one-off `medusa exec` update instead of reseeding.
10. Create an admin user (`npx medusa user ...`) and attach `pp_fluidpay_fluidpay` to the USA region via Admin UI or Admin API (see CLAUDE.md).
11. Hand the publishable API key and FluidPay public key off to the storefront's `.env.production` — see [`ta-strike-arena-website/README.md`](../ta-strike-arena-website/README.md#deploy-to-github-pages).
